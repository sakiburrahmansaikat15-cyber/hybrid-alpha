...
import { Plus, Search, Edit, Trash2, X, Check, Loader, ChevronLeft, ChevronRight, MoreVertical, Calendar } from 'lucide-react';
...
const LeaveTypes = () => {
  ...
  const fetchLeaveTypes = useCallback(
    async (page = 1, perPage = 10, keyword = '') => {
      ...
      try {
        ...
        const leaveTypeData = res.pagination?.data || [];
        const formattedLeaveTypes = leaveTypeData.map((item) => ({
          id: item.id,
          name: item.name || '—',
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchLeaveTypes(1, pagination.per_page, searchTerm);
    }, 500);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchLeaveTypes(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination((prev) => ({ ...prev, per_page: limit }));
    fetchLeaveTypes(1, limit, searchTerm);
  };
  ...
  const openModal = (leaveType = null) => {
    if (leaveType) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editingLeaveType) {
        response = await axios.post(`${LEAVE_TYPES_API}/${editingLeaveType.id}`, formData);
        showNotification('Leave type updated successfully');
      } else {
        response = await axios.post(LEAVE_TYPES_API, formData);
      ...
      handleApiError(error, 'Failed to save leave type');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this leave type permanently?')) return;
    setOperationLoading(`delete-${id}`);
    try {
      await axios.delete(`${LEAVE_TYPES_API}/${id}`);
      showNotification('Leave type deleted successfully');
      ...
      fetchLeaveTypes(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    return status === 'active' ? (
      ...
      <span className="px-3 py-1 bg-red-500/20 text-red-400 rounded-full text-xs font-medium">Inactive</span>
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage different types of employee leaves</p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={22} /> Add New Leave Type
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: 'Total Leave Types', value: stats.total, icon: Calendar, color: 'blue' },
          ].map((s, i) => (
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold">{leaveType.name}</h3>
                      <p className="text-sm text-gray-400">ID: {leaveType.id}</p>
                    </div>
                  </div>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setActionMenu(actionMenu === leaveType.id ? null : leaveType.id);
                    }}
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center text-gray-300">
                    <span>Status</span>
                    {getStatusBadge(leaveType.status)}
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-2 mt-6 pt-4 border-t border-gray-700/30">
                  ...
                  <button
                    ...
                    disabled={operationLoading === `delete-${leaveType.id}`}
        ...
        {leaveTypes.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">
              {searchTerm ? 'No leave types found' : 'No leave types created yet'}
            </h3>
            <p className="text-gray-400 mb-8">
              {searchTerm ? 'Try different keywords' : 'Create your first leave type to get started'}
            </p>
            {!searchTerm && (
              <button
                onClick={() => openModal()}
              ...
              >
                <Plus className="inline mr-2" /> Create First Leave Type
              </button>
            )}
          </div>
        )}
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold mb-2">Maximum Days Allowed *</label>
                  <input
                    type="number"
                    name="max_days"
                    ...
                    placeholder="e.g. 21"
                    className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${
                      errors.max_days ? 'border border-red-500' : ''
                    }`}
                  />
                  {errors.max_days && <p className="text-red-400 text-sm mt-1">{errors.max_days[0]}</p>}
                </div>
