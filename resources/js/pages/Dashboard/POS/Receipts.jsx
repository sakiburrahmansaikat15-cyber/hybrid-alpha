...
const API_URL = 'http://localhost:8000/api/pos/receipts';
...
const Receipts = () => {
  ...
  // Fetch receipts with server-side search & pagination
  const fetchReceipts = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      const receiptData = res.pagination?.data || [];
      const formattedReceipts = receiptData.map(item => ({
        id: item.id,
        receipt_no: item.receipt_no,
        sent_via: item.sent_via || null,
        sale: item.sale || null,
      ...
      handleApiError(error, 'Failed to fetch receipts');
      setReceipts([]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchReceipts(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchReceipts]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchReceipts(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination(prev => ({ ...prev, per_page: limit }));
    fetchReceipts(1, limit, searchTerm);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitData = {
        ...
        sent_via: formData.sent_via || null,
      };

      let response;
      if (editingReceipt) {
        response = await axios.post(`${API_URL}/${editingReceipt.id}`, submitData);
      } else {
        response = await axios.post(API_URL, submitData);
      }

      showNotification(
        response.data.message || `Receipt ${editingReceipt ? 'updated' : 'created'} successfully!`
      );
      fetchReceipts(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save receipt');
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      fetchReceipts(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
  ...
  const getSentViaLabel = (method) => {
    return method ? method.charAt(0).toUpperCase() + method.slice(1) : 'Not Sent';
  };

  const getSentViaColor = (method) => {
    switch (method) {
      case 'print': return 'text-gray-400';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Track and manage customer receipts for completed sales</p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={22} /> Issue New Receipt
          </button>
        </div>
        ...
        <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-gray-700/30">
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by receipt number or sale invoice..."
                value={searchTerm}
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-xl font-bold">{receipt.receipt_no}</h3>
                      <div className="flex items-center gap-2 text-sm text-gray-400 mt-1">
                        <FileText size={14} />
                        <span>{receipt.sale?.invoice_no || 'No Sale Linked'}</span>
                      </div>
                    </div>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === receipt.id ? null : receipt.id); }}
                ...
                <div className="space-y-3 mt-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`font-medium ${getSentViaColor(receipt.sent_via)}`}>
                      {getSentViaLabel(receipt.sent_via)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm text-gray-400 mt-6">
                  <div className="flex items-center gap-2">
                    <Calendar size={16} /> Issued: {formatDate(receipt.created_at)}
                  </div>
                </div>

                <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                  <span className="text-xs text-gray-500">Updated: {formatDate(receipt.updated_at)}</span>
                  <div className="flex gap-2">
                    <button onClick={() => openModal(receipt)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg">
                      <Edit size={14} />
                    </button>
                    <button onClick={() => handleDelete(receipt.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
              ...
              <AnimatePresence>
                {actionMenu === receipt.id && (
                  ...
                  >
                    <button onClick={() => { openModal(receipt); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm">
                      <Edit size={16} /> Edit
                    </button>
                    <button onClick={() => { handleDelete(receipt.id); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm">
        ...
        {receipts.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No receipts found' : 'No receipts issued yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Start issuing receipts for your sales'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-green-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Issue First Receipt
              </button>
            )}
          </div>
        )}
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold bg-gradient-to-r from-green-400 to-cyan-500 bg-clip-text text-transparent">
                  {editingReceipt ? 'Edit Receipt' : 'Issue New Receipt'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Associated Sale *</label>
                  <select
                    name="sale_id"
                  ...
                  >
                    ...
                    {sales.map(sale => (
                      <option key={sale.id} value={sale.id}>
                        {sale.invoice_no} ({sale.total_amount ? `$${parseFloat(sale.total_amount).toFixed(2)}` : 'N/A'})
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-green-500 outline-none"
                  />
                  {errors.receipt_no && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.receipt_no) ? errors.receipt_no[0] : errors.receipt_no}</p>}
                </div>

                <div>
                  <label className="block text-sm font-semibold mb-2">Sent Via (Optional)</label>
                  <select
                    name="sent_via"
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  >
                    ...
                    {editingReceipt ? 'Update' : 'Issue'} Receipt
                  </button>
                </div>
              </form>
            </motion.div>
