...
const API_URL = 'http://localhost:8000/api/pos/tax-rates';
...
const TaxRates = () => {
  ...
  // Fetch tax rates with server-side search & pagination
  const fetchTaxRates = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      const rateData = res.pagination?.data || [];
      const formattedRates = rateData.map(item => ({
        id: item.id,
        name: item.name,
        rate: item.rate,
      ...
      handleApiError(error, 'Failed to fetch tax rates');
      setTaxRates([]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchTaxRates(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchTaxRates]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchTaxRates(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination(prev => ({ ...prev, per_page: limit }));
    fetchTaxRates(1, limit, searchTerm);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingRate) {
        response = await axios.post(`${API_URL}/${editingRate.id}`, submitData);
      } else {
        response = await axios.post(API_URL, submitData);
      }

      showNotification(
        response.data.message || `Tax rate ${editingRate ? 'updated' : 'created'} successfully!`
      );
      fetchTaxRates(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save tax rate');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this tax rate permanently?')) return;
    ...
    try {
      ...
      fetchTaxRates(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage tax rates for your POS system</p>
          </div>
          <button
            onClick={() => openModal()}
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === rate.id ? null : rate.id); }}
                ...
                <div className="mb-6 text-center">
                  <p className="text-4xl font-bold text-cyan-400">{rate.rate}%</p>
                  <p className="text-sm text-gray-400 mt-1">Tax Rate</p>
                </div>

                <div className="space-y-2 text-sm text-gray-400">
                  <div className="flex items-center gap-2">
                    <Calendar size={16} /> Created: {formatDate(rate.created_at)}
                  </div>
                </div>

                <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                  <span className="text-xs text-gray-500">Updated: {formatDate(rate.updated_at)}</span>
                  <div className="flex gap-2">
                    <button onClick={() => openModal(rate)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg">
                      <Edit size={14} />
                    </button>
                    <button onClick={() => handleDelete(rate.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
              ...
              <AnimatePresence>
                {actionMenu === rate.id && (
                  ...
                  >
                    <button onClick={() => { openModal(rate); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm">
                      <Edit size={16} /> Edit
                    </button>
                    <button onClick={() => handleDelete(rate.id)} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm">
        ...
        {taxRates.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No tax rates found' : 'No tax rates yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Create your first tax rate'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Create First Tax Rate
              </button>
            )}
          </div>
        )}
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Tax Rate Name *</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                ...
                <div>
                  ...
                  <input
                    ...
                    step="0.01"
                    placeholder="e.g., 20"
                    className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.rate ? 'border-red-500' : ''}`}
                  />
                  {errors.rate && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.rate) ? errors.rate[0] : errors.rate}</p>}
                </div>
