...
const API_URL = 'http://localhost:8000/api/pos/sale-payments';
const SALES_API_URL = 'http://localhost:8000/api/pos/sales';
const PAYMENT_METHODS_API_URL = 'http://localhost:8000/api/pos/payment-methods';
...
const SalePayments = () => {
  ...
  // Fetch sale payments
  const fetchPayments = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      const paymentData = res.pagination?.data || [];
      const formattedPayments = paymentData.map(item => ({
        id: item.id,
        sale: item.sale || null,
        payment_method: item.payment_method || null, // matches your API: "payment_method"
        amount: item.amount,
      ...
      handleApiError(error, 'Failed to fetch sale payments');
      setPayments([]);
  ...
  // Fetch sales and payment methods for dropdowns
  ...
  const fetchPaymentMethods = useCallback(async () => {
    try {
      const response = await axios.get(PAYMENT_METHODS_API_URL);
      const res = response.data;
      const methodData = res.pagination?.data || res.data || [];
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchPayments(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchPayments]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchPayments(newPage, pagination.per_page, searchTerm);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification(
        response.data.message || `Sale payment ${editingPayment ? 'updated' : 'created'} successfully!`
      );
      fetchPayments(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save sale payment');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this sale payment permanently?')) return;
    setOperationLoading(`delete-${id}`);
    try {
      await axios.delete(`${API_URL}/${id}`);
      showNotification('Sale payment deleted successfully');
      ...
      fetchPayments(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Track and manage payments received for sales</p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={22} /> Record New Payment
          </button>
        </div>
        ...
        <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-gray-700/30">
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by reference, invoice or payment method..."
                value={searchTerm}
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === payment.id ? null : payment.id); }}
                ...
                {/* Extra visibility badge */}
                {payment.payment_method && (
                  <div className="mb-4">
                    <span className="inline-block px-4 py-1.5 rounded-full text-sm font-medium bg-blue-500/20 text-blue-400 border border-blue-500/30">
                      Paid via {payment.payment_method.name}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-3 mt-4">
                  {payment.sale && (
                    <div className="flex items-center gap-3 text-sm text-gray-300">
                      <Receipt size={16} />
                      <span>Invoice: {payment.sale.invoice_no}</span>
                    </div>
                  )}
                ...
                <div className="space-y-2 text-sm text-gray-400 mt-6">
                  <div className="flex items-center gap-2">
                    <Calendar size={16} /> Created: {formatDate(payment.created_at)}
                  </div>
                </div>

                <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                  <span className="text-xs text-gray-500">Updated: {formatDate(payment.updated_at)}</span>
                  <div className="flex gap-2">
                    <button onClick={() => openModal(payment)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg">
                      <Edit size={14} />
                    </button>
                    <button onClick={() => handleDelete(payment.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
              ...
              <AnimatePresence>
                {actionMenu === payment.id && (
                  ...
                  >
                    <button onClick={() => { openModal(payment); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm">
                      <Edit size={16} /> Edit
                    </button>
                    <button onClick={() => handleDelete(payment.id)} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm">
        ...
        {payments.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No payments found' : 'No sale payments yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Record the first payment for a sale'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Record First Payment
              </button>
            )}
          </div>
        )}
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  {errors.payment_method_id && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.payment_method_id) ? errors.payment_method_id[0] : errors.payment_method_id}</p>}
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold mb-2">Reference No (Optional)</label>
                  <input
                    type="text"
                    name="reference_no"
                    ...
                    className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.reference_no ? 'border-red-500' : ''}`}
                  />
                  {errors.reference_no && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.reference_no) ? errors.reference_no[0] : errors.reference_no}</p>}
                </div>
