...
import {
  ...
  Calendar, Loader, ChevronLeft, ChevronRight, Shield, Tag, Percent
} from 'lucide-react';

const API_URL = 'http://localhost:8000/api/crm/opportunity-stages';
...
const OpportunityStage = () => {
  ...
  const fetchStages = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      const stageData = res.pagination?.data || [];
      const formatted = stageData.map(item => ({
        id: item.id,
        name: item.name,
        probability: item.probability ?? 0,
        order: item.order ?? 0,
        opportunities_count: item.opportunities?.length || 0,
      ...
      handleApiError(error, 'Failed to fetch opportunity stages');
      setStages([]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchStages(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchStages]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchStages(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination(prev => ({ ...prev, per_page: limit }));
    fetchStages(1, limit, searchTerm);
  };

  const resetForm = () => {
    setFormData({ name: '', probability: 0, order: 0 });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingStage) {
        response = await axios.post(`${API_URL}/${editingStage.id}`, data);
      } else {
        response = await axios.post(API_URL, data);
      }

      showNotification(response.data.message || `Opportunity stage ${editingStage ? 'updated' : 'created'} successfully!`);
      fetchStages(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save opportunity stage');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this opportunity stage permanently?')) return;
    setOperationLoading(`delete-${id}`);
    try {
      await axios.delete(`${API_URL}/${id}`);
      showNotification('Opportunity stage deleted successfully');
      ...
      fetchStages(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
  ...
  const stats = {
    ...
    withOpportunities: stages.filter(s => s.opportunities_count > 0).length
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage the stages of your sales pipeline</p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={22} /> Add New Stage
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: 'Total Stages', value: stats.total, icon: Tag, color: 'blue' },
            { label: 'With Opportunities', value: stats.withOpportunities, icon: Shield, color: 'purple' },
          ].map((s, i) => (
        ...
        <div className="bg-gray-800/30 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-gray-700/30">
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search by name, probability or order..."
                value={searchTerm}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                <option value="order">Order (Pipeline)</option>
              ...
              >
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === stage.id ? null : stage.id); }}
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/20 text-green-400 border border-green-500/30 text-sm font-bold">
                      <Percent size={16} />
                      {stage.probability}%
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-purple-500/20 text-purple-400 border border-purple-500/30">
                      {stage.opportunities_count} {stage.opportunities_count === 1 ? 'opportunity' : 'opportunities'}
                    </span>
                  </div>

                  <div className="space-y-2 text-sm text-gray-400">
                    <div className="flex items-center gap-2"><Shield size={16} /> ID: #{stage.id}</div>
                    <div className="flex items-center gap-2"><Calendar size={16} /> Created: {formatDate(stage.created_at)}</div>
                  </div>

                  <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                    <span className="text-xs text-gray-500">Updated: {formatDate(stage.updated_at)}</span>
                    <div className="flex gap-2">
                      <button onClick={() => openModal(stage)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg"><Edit size={14} /></button>
                      <button onClick={() => handleDelete(stage.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg"><Trash2 size={14} /></button>
                    </div>
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {actionMenu === stage.id && (
                    ...
                    >
                      <button onClick={() => { openModal(stage); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm"><Edit size={16} /> Edit</button>
                      <button onClick={() => handleDelete(stage.id)} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm"><Trash2 size={16} /> Delete</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
        ...
        {stages.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No opportunity stages found' : 'No opportunity stages yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Create your first stage in the sales pipeline'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Create First Stage
              </button>
            )}
          </div>
        )}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-800 rounded-3xl p-8 max-w-2xl w-full border border-gray-700" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-cyan-500 bg-clip-text text-transparent">
                  {editingStage ? 'Edit Opportunity Stage' : 'Create New Opportunity Stage'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Prospecting, Qualification, Proposal, Won, Lost"
                  />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="50"
                    />
                    <p className="text-xs text-gray-400 mt-2">0 = no chance, 100 = won</p>
                  ...
                  <div>
                    <label className="block text-sm font-semibold mb-2">Order in Pipeline</label>
                    <input
                      type="number"
                      name="order"
