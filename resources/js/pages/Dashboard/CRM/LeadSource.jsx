...
import {
  ...
  Eye, EyeOff, Tag, Calendar, Loader, ChevronLeft, ChevronRight, Shield
} from 'lucide-react';

const API_URL = 'http://localhost:8000/api/crm/lead-sources';
...
const LeadSource = () => {
  ...
  const fetchSources = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      handleApiError(error, 'Failed to fetch lead sources');
      setSources([]);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      fetchSources(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchSources]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchSources(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination(prev => ({ ...prev, per_page: limit }));
    fetchSources(1, limit, searchTerm);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingSource) {
        response = await axios.post(`${API_URL}/${editingSource.id}`, data);
      } else {
        response = await axios.post(API_URL, data);
      }

      showNotification(response.data.message || `Lead source ${editingSource ? 'updated' : 'created'} successfully!`);
      fetchSources(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save lead source');
    } finally {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this lead source permanently?')) return;
    setOperationLoading(`delete-${id}`);
    try {
      await axios.delete(`${API_URL}/${id}`);
      showNotification('Lead source deleted successfully');
      ...
      fetchSources(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
  ...
  const toggleStatus = async (source) => {
    ...
    setOperationLoading(`status-${source.id}`);
    try {
      await axios.post(`${API_URL}/${source.id}`, {
        name: source.name,
        description: source.description,
        status: newStatus
      });

      showNotification(`Lead source ${newStatus ? 'activated' : 'deactivated'}`);
      fetchSources(pagination.current_page, pagination.per_page, searchTerm);
    } catch (error) {
  ...
  const stats = {
    ...
    active: sources.filter(s => s.status).length,
    inactive: sources.filter(s => !s.status).length
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage where your leads are coming from</p>
          </div>
          <button
            onClick={() => openModal()}
          ...
          >
            <Plus size={22} /> Add New Source
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: 'Total Sources', value: stats.total, icon: Tag, color: 'blue' },
        ...
        >
          {sources
            .filter(s => filterStatus === 'all' || (filterStatus === 'active' ? s.status : !s.status))
            .sort((a, b) => {
              ...
              >
                <div className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); setActionMenu(actionMenu === source.id ? null : source.id); }}
                  ...
                  <p className="text-gray-300 mb-4 line-clamp-2">{source.description}</p>

                  <div className="flex flex-wrap gap-2 mb-4">
                    <button
                      onClick={() => toggleStatus(source)}
                      disabled={operationLoading === `status-${source.id}`}
                      className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold ${source.status ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}
                    >
                      {operationLoading === `status-${source.id}` ? <Loader size={12} className="animate-spin" /> : null}
                  ...
                  <div className="space-y-2 text-sm text-gray-400">
                    <div className="flex items-center gap-2"><Shield size={16} /> ID: #{source.id}</div>
                    <div className="flex items-center gap-2"><Calendar size={16} /> Created: {formatDate(source.created_at)}</div>
                  </div>

                  <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                    <span className="text-xs text-gray-500">Updated: {formatDate(source.updated_at)}</span>
                    <div className="flex gap-2">
                      <button onClick={() => openModal(source)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg"><Edit size={14} /></button>
                      <button onClick={() => handleDelete(source.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg"><Trash2 size={14} /></button>
                    </div>
                  </div>
                </div>

                <AnimatePresence>
                  {actionMenu === source.id && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                    ...
                    >
                      <button onClick={() => { openModal(source); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm"><Edit size={16} /> Edit</button>
                      <button onClick={() => toggleStatus(source)} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm">
                        {source.status ? <EyeOff size={16} /> : <Eye size={16} />}
                        {source.status ? 'Deactivate' : 'Activate'}
                      </button>
                      <button onClick={() => handleDelete(source.id)} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm"><Trash2 size={16} /> Delete</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
        ...
        {sources.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No lead sources found' : 'No lead sources yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Create your first lead source'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Create First Source
              </button>
            )}
          </div>
        )}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-800 rounded-3xl p-8 max-w-2xl w-full border border-gray-700" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-cyan-500 bg-clip-text text-transparent">
                  {editingSource ? 'Edit Lead Source' : 'Create New Lead Source'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Optional details about this lead source..."
                  />
                </div>
