...
import {
  ...
  Globe, Mail, MessageSquare, DollarSign, Zap, Calendar, Target
} from 'lucide-react';

const API_URL = '/api/crm/campaigns';
...
const Campaign = () => {
  ...
  const [formData, setFormData] = useState({ name: '', type: 'email', start_date: '', end_date: '', budget: '' });
  ...
  const fetchCampaigns = useCallback(async (page = 1, keyword = searchTerm) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    const timer = setTimeout(() => fetchCampaigns(1), 500);
  ...
  const openModal = (campaign = null) => {
    if (campaign) {
      ...
      setFormData({ name: '', type: 'email', start_date: '', end_date: '', budget: '' });
    }
    setErrors({});
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCampaign) {
        await axios.put(`${API_URL}/${editingCampaign.id}`, formData);
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <header className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight dark:text-white mb-1 flex items-center gap-3">
              <Target className="text-rose-500" /> Campaigns
            </h1>
            <p className="text-sm text-slate-500">Plan and track your marketing initiatives</p>
          </div>
        ...
        {loading && campaigns.length === 0 ? (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => openModal(campaign)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 hover:text-rose-600 transition-colors">
                        <Edit size={16} />
                      </button>
                      <button onClick={() => handleDelete(campaign.id)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-400 hover:text-rose-600 transition-colors">
                        {operationLoading === campaign.id ? <Loader className="animate-spin" size={16} /> : <Trash2 size={16} />}
                      </button>
                    </div>
                  </div>

                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 truncate">{campaign.name}</h3>
                  <div className="text-xs font-bold uppercase tracking-wide text-slate-400 mb-4">{campaign.type} Campaign</div>

                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500 flex items-center gap-1"><Calendar size={12} /> Duration</span>
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-slate-500 flex items-center gap-1"><DollarSign size={12} /> Budget</span>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <p className="font-semibold text-lg">No Campaigns Found</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
                <h2 className="text-xl font-bold dark:text-white">{editingCampaign ? 'Edit Campaign' : 'New Campaign'}</h2>
