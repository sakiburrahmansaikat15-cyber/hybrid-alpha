...
import {
  ...
  Calendar, Loader, ChevronLeft, ChevronRight, Shield, Tag, User, Mail, Phone, Building, Star
} from 'lucide-react';

const API_URL = 'http://localhost:8000/api/crm/leads';
const SOURCES_URL = 'http://localhost:8000/api/crm/lead-sources';
const STATUSES_URL = 'http://localhost:8000/api/crm/lead-statuses';
...
const Lead = () => {
  ...
  const fetchLeads = useCallback(async (page = 1, perPage = 10, keyword = '') => {
    setLoading(true);
    try {
      ...
      const leadData = res.pagination?.data || [];
      const formatted = leadData.map(item => ({
        id: item.id,
        name: item.name,
        ...
        score: item.score || 0,
        lead_source: item.lead_source || { name: 'Unknown' },
        lead_status: item.lead_status || { name: 'Unknown', color_code: '#6B7280' },
  ...
  const fetchSourcesAndStatuses = useCallback(async () => {
    try {
      const [sourcesRes, statusesRes] = await Promise.all([
        axios.get(SOURCES_URL),
        axios.get(STATUSES_URL)
      ]);

      setSources(sourcesRes.data.pagination?.data || []);
      setStatuses(statusesRes.data.pagination?.data || []);
    } catch (error) {
      console.error('Failed to load sources/statuses', error);
    }
  }, []);

  useEffect(() => {
    const timer = setTimeout(() => {
      fetchLeads(1, pagination.per_page, searchTerm);
    }, 500);
    return () => clearTimeout(timer);
  }, [searchTerm, pagination.per_page, fetchLeads]);
  ...
  const handlePageChange = (newPage) => {
    ...
    fetchLeads(newPage, pagination.per_page, searchTerm);
  };

  const handleLimitChange = (newLimit) => {
    const limit = parseInt(newLimit);
    setPagination(prev => ({ ...prev, per_page: limit }));
    fetchLeads(1, limit, searchTerm);
  };
  ...
  const openModal = (lead = null) => {
    if (lead) {
      ...
      setFormData({
        ...
        score: lead.score || 0,
        lead_source_id: lead.lead_source?.id || '',
        lead_status_id: lead.lead_status?.id || ''
      });
    } else {
      resetForm();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingLead) {
        response = await axios.post(`${API_URL}/${editingLead.id}`, data);
      } else {
        response = await axios.post(API_URL, data);
      }

      showNotification(response.data.message || `Lead ${editingLead ? 'updated' : 'created'} successfully!`);
      fetchLeads(pagination.current_page, pagination.per_page, searchTerm);
      closeModal();
    } catch (error) {
      handleApiError(error, 'Failed to save lead');
    } finally {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      fetchLeads(targetPage || 1, pagination.per_page, searchTerm);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
          <div>
            ...
            <p className="text-gray-400 mt-2">Manage your leads and their information</p>
          </div>
          <button
            onClick={() => openModal()}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            ...
            { label: 'High Score (≥80)', value: stats.highScore, icon: Star, color: 'yellow' },
          ].map((s, i) => (
              ...
              >
                <div className="p-6">
                  ...
                  <div className="space-y-2 mb-4 text-sm text-gray-300">
                    ...
                    <div className="flex items-center gap-2"><Building size={16} /> {lead.company}</div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    <span
                      ...
                      style={{
                        backgroundColor: `${lead.lead_status.color_code}30`,
                        color: lead.lead_status.color_code,
                        border: `1px solid ${lead.lead_status.color_code}50`
                      }}
                    >
                  ...
                  <div className="space-y-2 text-sm text-gray-400">
                    <div className="flex items-center gap-2"><Shield size={16} /> ID: #{lead.id}</div>
                    <div className="flex items-center gap-2"><Calendar size={16} /> Created: {formatDate(lead.created_at)}</div>
                  </div>

                  <div className="flex justify-between items-center mt-6 pt-4 border-t border-gray-700/30">
                    <span className="text-xs text-gray-500">Updated: {formatDate(lead.updated_at)}</span>
                    <div className="flex gap-2">
                      <button onClick={() => openModal(lead)} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 rounded-lg"><Edit size={14} /></button>
                      <button onClick={() => handleDelete(lead.id)} className="p-2 bg-red-500/20 hover:bg-red-500/40 rounded-lg"><Trash2 size={14} /></button>
                    </div>
                  </div>
                </div>
                ...
                <AnimatePresence>
                  {actionMenu === lead.id && (
                    ...
                    >
                      <button onClick={() => { openModal(lead); setActionMenu(null); }} className="w-full text-left px-4 py-2 hover:bg-gray-700 flex items-center gap-3 text-sm"><Edit size={16} /> Edit</button>
                      <button onClick={() => handleDelete(lead.id)} className="w-full text-left px-4 py-2 hover:bg-red-500/20 text-red-400 flex items-center gap-3 text-sm"><Trash2 size={16} /> Delete</button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
        ...
        {leads.length === 0 && !loading && (
          <div className="text-center py-20">
            ...
            <h3 className="text-2xl font-bold mb-3">{searchTerm ? 'No leads found' : 'No leads yet'}</h3>
            <p className="text-gray-400 mb-8">{searchTerm ? 'Try different keywords' : 'Create your first lead'}</p>
            {!searchTerm && (
              <button onClick={() => openModal()} className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-3 rounded-xl font-bold">
                <Plus className="inline mr-2" /> Create First Lead
              </button>
            )}
          </div>
        )}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.company ? 'border-red-500' : ''}`}
                    placeholder="Company name"
                  />
                  {errors.company && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.company) ? errors.company[0] : errors.company}</p>}
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <label className="block text-sm font-semibold mb-2">Lead Score</label>
                    <input
                      type="number"
                      name="score"
                      ...
                      min="0"
                      max="100"
                      className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.score ? 'border-red-500' : ''}`}
                      placeholder="0-100"
                    />
                    {errors.score && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.score) ? errors.score[0] : errors.score}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleInputChange}
                      required
                      className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.lead_source_id ? 'border-red-500' : ''}`}
                    >
                    ...
                    {errors.lead_source_id && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.lead_source_id) ? errors.lead_source_id[0] : errors.lead_source_id}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={handleInputChange}
                      required
                      className={`w-full px-4 py-3 bg-gray-700/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none ${errors.lead_status_id ? 'border-red-500' : ''}`}
                    >
                    ...
                    {errors.lead_status_id && <p className="text-red-400 text-sm mt-1">{Array.isArray(errors.lead_status_id) ? errors.lead_status_id[0] : errors.lead_status_id}</p>}
                  </div>
                </div>
