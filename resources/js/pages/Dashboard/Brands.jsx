...
const Brands = () => {
  ...
  const handleApiError = useCallback((error, defaultMessage) => {
    ...
    if (error.response?.status === 404) {
      showNotification('Brand not found.', 'error');
      return { _general: 'Brand not found.' };
  ...
  const fetchBrands = useCallback(async () => {
    ...
    try {
      ...
      setPagination(prev => ({
        ...prev,
        last_page: Math.ceil(brandsData.length / prev.per_page),
        total: brandsData.length
      }));

    } catch (error) {
      console.error('Error fetching brands:', error);
      handleApiError(error, 'Error fetching brands');
      setBrands([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingBrand) {
        // For update - use POST with _method=PUT for file uploads
      ...
      if (response.data.success) {
        showNotification(
          response.data.message || `Brand ${editingBrand ? 'updated' : 'created'} successfully!`,
          'success'
        );
        fetchBrands();
        closeModal();
      } else {
        throw new Error(response.data.message || `Failed to ${editingBrand ? 'update' : 'create'} brand`);
      }
    } catch (error) {
      console.error('Submit Error:', error);
      const err = handleApiError(error, 'Error saving brand');
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.data.success) {
        showNotification(response.data.message || 'Brand deleted successfully!', 'success');
        fetchBrands();
      } else {
        throw new Error(response.data.message || 'Failed to delete brand');
      }
    } catch (error) {
      handleApiError(error, 'Error deleting brand');
    } finally {
      setOperationLoading(null);
      setActionMenu(null);
    }
  ...
  const toggleStatus = async (brand) => {
    ...
    try {
      ...
      if (brand.image) {
        // If there's an existing image, we need to maintain it
        formData.append('existing_image', brand.image);
      }

      const response = await axios.post(`${API_URL}/${brand.id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        }
      });
      ...
      if (response.data.success) {
        showNotification(`Brand ${newStatus ? 'activated' : 'deactivated'} successfully!`, 'success');
        fetchBrands();
      } else {
  ...
  const handleLimitChange = (newLimit) => {
    ...
    setPagination(prev => ({
      ...
      per_page: limit,
      current_page: 1,
      last_page: Math.ceil(prev.total / limit)
    }));
  };
  ...
  const filteredBrands = React.useMemo(() => {
    let filtered = brands.filter(brand => {
      const matchesSearch = brand.name?.toLowerCase().includes(searchTerm.toLowerCase()) || false;
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'status':
          return (b.status ? 1 : -1) - (a.status ? 1 : -1);
        case 'newest':
    ...
    return filtered;
  }, [brands, searchTerm, filterStatus, sortBy]);
  ...
  }, [filteredBrands, pagination.current_page, pagination.per_page]);

  // Stats calculation
  const stats = {
    total: brands.length,
    active: brands.filter(brand => brand.status).length,
    inactive: brands.filter(brand => !brand.status).length
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex-1">
              ...
              <p className="text-gray-400 mt-2 text-lg">Manage product brands and their information</p>
            </div>
            <motion.button
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-gray-800/40 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/40 hover:border-blue-500/30 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">Active Brands</p>
          ...
          <div className="bg-gray-800/40 backdrop-blur-sm rounded-2xl p-6 border border-gray-700/40 hover:border-blue-500/30 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-400 text-sm font-medium">Inactive Brands</p>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="relative">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setActionMenu(actionMenu === brand.id ? null : brand.id);
                      }}
                    ...
                    <AnimatePresence>
                      {actionMenu === brand.id && (
                        ...
                        >
                          ...
                          >
                            {operationLoading === `status-${brand.id}` ? (
                          ...
                          <button
                            ...
                            disabled={operationLoading === `delete-${brand.id}`}
                          ...
                          >
                            {operationLoading === `delete-${brand.id}` ? (
                              <Loader size={16} className="animate-spin" />
                            ) : (
                              <Trash2 size={16} />
                            )}
                ...
                <div className="mb-4">
                  ...
                  >
                    {operationLoading === `status-${brand.id}` ? (
                      ...
                      <div className={`w-2 h-2 rounded-full ${brand.status ? 'bg-green-400' : 'bg-red-400'}`}></div>
                    )}
